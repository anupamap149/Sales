    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute("INSERT INTO product (name, category, quantity, price) VALUES (?,?,?,?)",(nm, category, quantity, price))
...
def editrec():
    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute("UPDATE product SET name='"+nm+"', category='"+category+"', price='"+price+"', quantity='"+quantity+"' WHERE rowid="+rowid)

                con.commit()
                msg = "Record successfully edited in the database"
        except:
            con.rollback()
            msg = "Error in the Edit: UPDATE product SET name="+nm+", category="+category+", price="+price+", quantity="+quantity+" WHERE rowid="+rowid
